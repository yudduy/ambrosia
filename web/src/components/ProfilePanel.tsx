    ...
    <aside className={`profile-panel ${open ? "profile-panel--open" : ""}`} aria-hidden={!open} aria-label="Health profile">
      ...
      {form && (
        <div className="profile-form">
          <TextInput id="profile-goal" labelText="Goal" placeholder="Build fitness or improve sleep" value={form.goal ?? ""} onChange={(event) => update("goal", event.target.value || null)} />
          <TextInput id="profile-horizon" labelText="Time frame" placeholder="Next 12 weeks" value={form.time_horizon ?? ""} onChange={(event) => update("time_horizon", event.target.value || null)} />
          <TextInput id="profile-training" labelText="Workouts per week" placeholder="Four" value={form.training_frequency ?? ""} onChange={(event) => update("training_frequency", event.target.value || null)} />
          <TextArea id="profile-preferences" labelText="Diet" helperText="Separate items with commas" rows={2} value={preferences} onChange={(event) => setPreferences(event.target.value)} />
          <TextArea id="profile-constraints" labelText="Constraints" helperText="Schedule, injuries, or foods to avoid" rows={2} value={constraints} onChange={(event) => setConstraints(event.target.value)} />
